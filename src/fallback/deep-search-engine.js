 ...
 */
async function searchConfiguredSites(query, configuredUrls, product = 'comunicai', openaiClient, capture) {
  const startTime = Date.now();
  ...
  // ── Cattura citations per il widget (roadmap #1 H) ───────────────────────────
  // Le pagine più rilevanti diventano le "fonti cliccabili" mostrate sotto la
  // risposta. Funziona anche nel percorso di fallback (nessun KB vettoriale).
  if (capture) {
    const tokens = tokenize(query);
    const scored = allCrawledPages
      .map(p => ({ title: p.title || p.pageUrl, url: p.pageUrl, score: scorePageRelevance(p, tokens) }))
      .sort((a, b) => b.score - a.score);
    const top = scored.filter(p => p.score > 0).slice(0, 5);
    const citations = top.length
      ? top
      : scored.slice(0, 5);
    // Contenuto reale del sito → consideriamo la risposta "verificata".
    capture.citations = citations.map(c => ({ title: c.title, url: c.url }));
    capture.confidence = citations.length ? 0.7 : 0;
  }
